...
public class RegionController {
    ...
    public NewDonResult query(Region region, Integer page, Integer rows) {
        if (null == page || page < 0) {
            page = 1;
        }
        if (null == rows || rows < 0) {
    ...
    public NewDonResult insert(@Validated(Insert.class) @RequestBody Region region, BindingResult bindingResult) {
        ...
        if (insert) {
            return NewDonResult.build(200, "OK", region.getId());
        } else {
    ...
    public NewDonResult update(@Validated(Update.class) @RequestBody Region region, BindingResult bindingResult) {
        if (bindingResult.hasErrors()) {
            return NewDonResult.build(400, "FAILED", bindingResult.getFieldError().getDefaultMessage());
        }
